...
class Formula :
    ...
    def ver_comb(self, lista_descriptores):
        '''
        Visualiza una fórmula A (como string en notación inorder) usando los descriptores en la lista lista_descriptores
        '''
        vis = []
        A = str(self)
        contador = 0
    ...
    def SATtableaux(self):
        ...
        while len(frontera) > 0:
            estado = frontera.pop(0)
            # print('Frontera:\n', [str(x) for x in frontera])
            # print('Estado a explorar:\n', str(estado))
            hijos = estado.expandir()
            for a in hijos:
                if a != None:
                    res = a.es_hoja()
